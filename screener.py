...
def kosdaq_pct():
    ...
    try:
        return float(str(j.get("fluctuationsRatio", "0")).replace(",", ""))
    except Exception:
        return 0.0
...
if NOW.hour >= 15:
    ...
    if last_done != TODAY:
        try:
            lines = [f"📊 <b>{NOW.strftime('%m/%d')} 마감 요약</b>  코스닥 {KQ:+.1f}%"]
            alerted = set()
            if os.path.exists(TRACK):
                tr = pd.read_csv(TRACK, dtype={"code": str})
...
sent = {k: v for k, v in sent.items() if str(v).startswith(TODAY) or
        (isinstance(v, dict) and str(v.get("t", "")).startswith(TODAY))}
...
for kind, tbl, minscore in [("돌파", breakout, 85), ("눌림목", pullback, 70)]:
    for _, r in tbl.iterrows():
        ...
        if rr < RR_MIN:
            print(f"제외 {r['name']}: 손익비 {rr:.1f} (기준 {RR_MIN})"); continue
        picked.append({"kind": kind, "r": r, "final": final, "note": note,
                       "sup": sup, "res": res, "rr": rr, "risk_pct": risk / r["종가"] * 100,
...
if picked:
    head = f"🔔 <b>{NOW.strftime('%H:%M')} 신호 {len(picked)}건</b>  코스닥 {KQ:+.1f}%"
    if KQ <= -1.5: head += " ⚠️약세장"
    blocks = []
    for p in picked:
        r = p["r"]; code = r["code"]
