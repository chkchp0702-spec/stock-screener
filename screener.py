...
print(f"{STAMP} | 수집 {len(rows)} → 매칭 {len(cur)} / 유니버스 {len(uni)} | 누락 {len(missing)}")
...
if PRE:
    ...
    if len(active) >= 3 and done != TODAY and HHMM >= 830:
        ...
        tg("\n".join(lines)); open(PREDONE, "w").write(TODAY)
    raise SystemExit(0)
...
COLS = ["시각","유형","code","name","알람가","점수","손익비","지지","저항","VWAP위","연속",
        "단골","코스닥","상승비율","당일등락","30분","60분","최고","최저","현재","청산알림"]
streak_loss, today_cnt, regulars = 0, 0, {}
exits = []
if os.path.exists(TRACK):
    try:
        ...
        regulars = Counter(tr["code"])
        px = dict(zip(cur["code"], cur["종가"]))
        vwp = dict(zip(cur["code"], cur["거래대금"] / cur["거래량"].replace(0, float("nan"))))
        for i, row in tr.iterrows():
            ...
            if str(row["시각"]).startswith(TODAY):
                ...
                res, sup = row.get("저항"), row.get("지지")
                vw = vwp.get(row["code"])
                if res and not pd.isna(res) and p >= float(res) and "익절" not in flag:
                    exits.append(("익절", row, p, chg)); flag += "익절"
                elif sup and not pd.isna(sup) and p <= float(sup) and "손절" not in flag:
                    exits.append(("손절", row, p, chg)); flag += "손절"
                elif vw and not pd.isna(vw) and p < vw and chg < -1 and "이탈" not in flag:
                    exits.append(("이탈", row, p, chg)); flag += "이탈"
                tr.at[i, "청산알림"] = flag
        ...
        print("추적 갱신 실패", e)
print(f"오늘 알람 {today_cnt}건 · 연속손실 {streak_loss}")

if exits:
    lines = [f"🔔 <b>{NOW.strftime('%H:%M')} 청산 신호</b>", ""]
    for kind, row, p, chg in exits[:6]:
        icon = {"익절": "💰", "손절": "🛑", "이탈": "📉"}[kind]
        lines.append(f"{icon} <b>{kind}</b> {link(row['code'], row['name'])}"
                     f"\n  {int(p):,}원 · 알람가 대비 {chg:+.1f}%")
    tg("\n".join(lines)); print("청산 알람", len(exits))
            ...
            if os.path.exists(TRACK):
                ...
                if len(t):
                    ...
                    for _, x in t.iterrows():
                        try:
                            tgt = (float(x["저항"]) / x["알람가"] - 1) * 100
                            stp = (float(x["지지"]) / x["알람가"] - 1) * 100
                            if x["최저"] <= stp: sim.append(stp)
                            elif x["최고"] >= tgt: sim.append(tgt)
                    ...
                    lines += [f"알람 {len(t)}건 · 종가 플러스 {win}건 ({win/len(t)*100:.0f}%)",
                              ...
                              f"💼 시뮬(지지손절·저항익절) 평균 <b>{sum(sim)/len(sim):+.2f}%</b>", ""]
...
# ── 내일 관심종목 (15:20 이후) ──
NEXTDONE = "data/next_done.txt"
if HHMM >= 1520:
    nd = open(NEXTDONE).read().strip() if os.path.exists(NEXTDONE) else ""
    if nd != TODAY:
        try:
            cand = cur[(cur["등락률"].between(2, 15)) & (cur["거래대금"] >= 3e9)]
            cand = cand.merge(uni[["code", "전일거래대금"]], on="code", how="left")
            cand["배율"] = cand["거래대금"] / cand["전일거래대금"]
            cand = cand[cand["배율"] >= 2].sort_values("거래대금", ascending=False)
            picks = []
            for _, r in cand.head(15).iterrows():
                ...
                if not (c.tail(5).mean() > c.tail(20).mean()): continue
                hi_t, lo_t = d["h"].iloc[-1], d["l"].iloc[-1]
                if hi_t > lo_t and (hi_t - r["종가"]) / (hi_t - lo_t) > 0.35: continue
                picks.append(r)
                if len(picks) >= 3: break
                time.sleep(0.2)
            if picks:
                lines = ["🌙 <b>내일 관심종목</b> (종가 강세·거래 급증·정배열)", ""]
                for r in picks:
                    lines.append(f"  {link(r['code'], r['name'])} {int(r['종가']):,}원 "
                                 f"{r['등락률']:+.1f}% · 평소 {r['배율']:.1f}배")
                tg("\n".join(lines))
            open(NEXTDONE, "w").write(TODAY)
        except Exception as e:
            print("내일예고 실패", e)

if today_cnt >= MAX_ALERTS:
    print("알람 상한 도달"); raise SystemExit(0)
if streak_loss >= 3:
    print("3연패 중단"); raise SystemExit(0)
...
def mins_between(f1, f2):
    t1 = datetime.strptime(f1[:13], "%Y%m%d_%H%M")
    t2 = datetime.strptime(f2[:13], "%Y%m%d_%H%M")
    return max((t2 - t1).total_seconds() / 60, 1)

gap1 = mins_between(reg[-2], reg[-1])
...
m["구간대금"] = m["거래대금"] - m["거래대금_p"]
m["분당대금"] = m["구간대금"] / gap1
m["가격변화"] = (m["종가"] / m["종가_p"] - 1) * 100
m["회전율"] = m["분당대금"] * 10 / m["시총"] * 10000
...
for k in range(2, 5):
    if len(reg) >= k + 1:
        try:
            a = pd.read_csv(f"data/snaps/{reg[-k]}", dtype={"code": str})
            b = pd.read_csv(f"data/snaps/{reg[-k-1]}", dtype={"code": str})
            g = mins_between(reg[-k-1], reg[-k])
            d = a[["code", "거래대금"]].merge(b[["code", "거래대금"]], on="code", suffixes=("", "_b"))
            d["pm"] = (d["거래대금"] - d["거래대금_b"]) / g
            base_list.append(d[["code", "pm"]])
        except Exception: pass
if base_list:
    bb = pd.concat(base_list).groupby("code", as_index=False)["pm"].mean()
    m = m.merge(bb, on="code", how="left")
    m["가속도"] = m["분당대금"] / m["pm"].clip(lower=1)
else:
    m["가속도"] = 1.0
    ...
    try:
        ...
        for c_, p_ in zip(s["code"], s["종가"]):
            ...
            if c_ not in dayhigh or p_ > dayhigh[c_]:
                dayhigh[c_] = p_; hightime[c_] = f
    except Exception: pass
m["당일고점"] = m["code"].map(dayhigh)
m["고점후"] = m["code"].map(lambda c: mins_between(hightime[c], reg[-1]) if c in hightime else 0)
m["고점대비"] = (m["종가"] / m["당일고점"] - 1) * 100

m = m[(m["구간대금"] > 3e8) & (m["회전율"].notna()) & (m["당일회전"] < 25)]
...
top = m.head(30)[["code","name","종가","등락률","가격변화","점수","회전율","전일대비","가속도","구간대금"]].round(2)
top.to_csv("data/signal_latest.csv", index=False, encoding="utf-8-sig")
print(top.head(8).to_string(index=False))

breakout = m[(m["점수"] >= 80) & (m["구간대금"] >= VOL_MIN)
             & (m["전일대비"] >= 2.0) & (m["가격변화"] >= 1.0)].head(8)
pullback = m[(m["점수"] >= 65) & (m["구간대금"] >= VOL_MIN * 0.3)
             & (m["전일대비"] >= 2.0) & (m["등락률"] >= 2)
...
for kind, tbl, minscore in [("돌파", breakout, 85), ("눌림목", pullback, 70)]:
    for _, r in tbl.iterrows():
        ...
        pr = sent.get(code)
        if pr:
            gap = (NOW - datetime.strptime(pr["t"], "%Y%m%d_%H%M").replace(tzinfo=KST)).total_seconds()
            if pr.get("kind") == kind and gap < 1800: continue
            if pr.get("kind") != kind and gap < 600: continue
        ...
        picked.append({"kind": kind, "r": r, "final": final, "note": note, "sup": sup,
                       ...
                       "streak": code in prev_ok, "reg": regulars.get(code, 0),
                       "news": news(code)})
        time.sleep(0.2)
...
if picked:
    ...
    for p in picked:
        ...
        reg_s = f" 🔁단골{p['reg']}회" if p["reg"] >= 3 else ""
        again = (f" · 어제 {r['전일등락']:+.0f}%"
                 if (not pd.isna(r.get("전일등락")) and r["전일등락"] >= 10) else "")
        t = f"{tag} <b>{link(code, r['name'])}</b> ({code}){star}{reg_s}"
        t += (f"\n  {int(r['종가']):,}원  당일 {r['등락률']:+.1f}%  {gap1:.0f}분 {r['가격변화']:+.1f}%{again}"
              f"\n  점수 {p['final']:.0f} · 전일대비 {r['전일대비']:.1f}배 · 분당 {r['분당대금']/1e8:.1f}억")
        if p["kind"] == "눌림목":
    ...
    nt = pd.DataFrame([{"시각": STAMP, "유형": p["kind"], "code": p["r"]["code"],
                        ...
                        "지지": round(p["sup"]) if p["sup"] else None,
                        "저항": round(p["res"]) if p["res"] else None,
                        "VWAP위": bool(p["r"]["VWAP위"]), "연속": p["streak"],
                        "단골": p["reg"], "코스닥": round(KQ, 2),
                        "상승비율": round(up_ratio, 0), "당일등락": p["r"]["등락률"],
                        "30분": None, "60분": None, "최고": 0.0, "최저": 0.0,
                        "현재": 0.0, "청산알림": ""} for p in picked])
    if os.path.exists(TRACK):
        try:
