...
for _, r in alert.iterrows():
    ...
    if last and (NOW - datetime.strptime(last, "%Y%m%d_%H%M").replace(tzinfo=KST)).total_seconds() < 1800:
        continue
    ok, bonus, note, sup, res = analyze(code, r["등락률"], r["종가"], dbg=first)
    if not ok:
        print(f"제외 {r['name']}: {note}"); first = False; continue
    final = r["점수"] + bonus
    if final < 85: first = False; continue
    picked.append({"r": r, "final": final, "note": note, "sup": sup, "res": res,
                   "sec": sector(code), "news": news(code)})
...
if picked:
    ...
    for p in picked:
        r = p["r"]; code = r["code"]
        t = f"<b>{r['name']}</b> ({code})" + (f" · {p['sec']}" if p["sec"] else "")
    ...
    # 추적 기록 추가
    newrows = [{"시각": STAMP, "code": p["r"]["code"], "name": p["r"]["name"],
                "알람가": p["r"]["종가"], "점수": round(p["final"], 1),
                "당일등락": p["r"]["등락률"], "30분": None, "60분": None,
                "최고": 0.0, "현재": 0.0} for p in picked]
    nt = pd.DataFrame(newrows)
    if os.path.exists(TRACK):
        try:
