...
def analyze(code, today_pct, price):
    d = daily(code)
    if d is None: return False, 0, "일봉 없음", None, None, False
    ...
    if today_pct > 15: return False, 0, "당일 +15% 초과", None, None, False
    if up5 > 30:       return False, 0, "5일 +30% 초과", None, None, False

    notes, bonus = [], 0
    if ma60 and ma5 > ma20 > ma60: bonus += 5; notes.append("정배열")
    elif ma5 > ma20:               bonus += 2; notes.append("5>20")
    newhigh = price > hi20
    if newhigh:   bonus += 5; notes.append("20일신고가")
    ...
    return True, bonus, " · ".join(notes), support, resist, newhigh

def sector(code, dbg=False):
    j = get(f"https://m.stock.naver.com/api/stock/{code}/integration", debug=dbg)
    if not j: return ""
    for k in ("industryCodeType", "industry", "sector"):
        v = j.get(k)
        if isinstance(v, dict):
            for kk in ("industryGroupKor", "name", "industryName"):
                if v.get(kk): return v[kk]
    ...
    try:
        ...
        for i, row in tr.iterrows():
            ...
            if str(row["시각"]).startswith(TODAY):
                prevmax = row.get("최고")
                tr.at[i, "최고"] = round(max(chg, prevmax if not pd.isna(prevmax) else -99), 2)
...
pullback = m[(m["점수"] >= 70) & (m["최근5분"] >= 5e8)
             & (m["전일대비"] >= 2.0) & (m["등락률"] >= 3)
             & (m["고점대비"].between(-10, -2)) & (m["고점후"] >= 20)
             & (m["가격변화"] >= 0.3)].head(5)
...
picked, first = [], True
for kind, tbl, minscore in [("돌파", breakout, 85), ("눌림목", pullback, 75)]:
    for _, r in tbl.iterrows():
        ...
        ok, bonus, note, sup, res, nh = analyze(code, r["등락률"], r["종가"])
        if not ok:
            print(f"제외 {r['name']}: {note}"); continue
        final = r["점수"] + bonus
        if final < minscore: continue
        picked.append({"kind": kind, "r": r, "final": final, "note": note,
                       "sup": sup, "res": res, "nh": nh,
                       "sec": sector(code, dbg=first), "news": news(code)})
        first = False; time.sleep(0.2)

if picked:
    cnt = Counter(p["sec"] for p in picked if p["sec"])
    hot = [s for s, n in cnt.items() if n >= 2]
    head = f"🔔 <b>{NOW.strftime('%H:%M')} 신호 {len(picked)}건</b>"
    if hot: head += "\n🔥 " + ", ".join(f"{s} {cnt[s]}종목" for s in hot)
    blocks = []
    for p in picked:
        r = p["r"]; code = r["code"]
        tag = "🚀" if p["kind"] == "돌파" else "🔄"
        t = f"{tag} <b>{r['name']}</b> ({code})" + (f" · {p['sec']}" if p["sec"] else "")
        ...
        if p["sup"]:
            if p["res"]:
                risk = r["종가"] - p["sup"]; rew = p["res"] - r["종가"]
                rr = f" (1:{rew/risk:.1f})" if risk > 0 else ""
                t += f"\n  🎯 지지 {int(p['sup']):,} / 저항 {int(p['res']):,}{rr}"
            else:
    ...
    nt = pd.DataFrame([{"시각": STAMP, "유형": p["kind"], "code": p["r"]["code"],
                        ...
                        "점수": round(p["final"], 1), "당일등락": p["r"]["등락률"],
